...
| `kimi-not-found.txt` | Synthesized from POSIX shell and Windows `cmd.exe` command-not-found output | Kimi CLI is missing from `PATH` during provider prerequisite checks. | Pattern id `kimi-not-found`; action should tell users to install Kimi CLI and rerun after `kimi` is available. |
| `kimi-not-signed-in.txt` | Synthesized from Kimi CLI credential/auth output | Kimi CLI is installed but no local credentials/session are available. | Pattern id `kimi-not-signed-in`; action should tell users to run `kimi login` or sign in before retrying. |
| `kimi-version-old.txt` | Synthesized from stale `kimi info` output | Kimi CLI is installed, signed in, but reports `kimi-cli version: 0.9.x`. | Pattern id `kimi-version-old`; action should tell users to upgrade Kimi CLI to the supported version. |
