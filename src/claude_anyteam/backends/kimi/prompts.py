...
Kimi exposes MCP tools as mcp_<server>_<tool>; the adapter configures the
...
def task_prompt(task, agent_name: str, team_name: str) -> str:
    return (
        ...
        f"# Subject\n{task.subject}\n\n# Description\n{task.description}\n\n"
        "# MCP tools available\nKimi built-in local tools are intentionally disabled. For shell, filesystem, edit, search, and web fetch work, use the mcp_anyteam_* shadow tools below so outputs are visible to the adapter. Use protocol tools only when useful; do not call them by default:\n"
...
def plan_prompt(task, *, tighten: bool, agent_name: str, team_name: str) -> str:
    ...
    return (
        f"{header}\n\n# Subject\n{task.subject}\n\n# Description\n{task.description}\n\n"
        "Protocol tools exist but should not be used while planning. If discussing execution, plan to use mcp_anyteam_shell/read_file/write_file/list_directory/edit_file/search/web_fetch rather than Kimi built-ins.\n"
